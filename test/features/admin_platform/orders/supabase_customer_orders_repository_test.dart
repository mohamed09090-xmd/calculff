  ...
  group('SupabaseCustomerOrdersRepository', () {
    ...
    test('last page has no cursor and loses no returned rows', () async {
      final rows = <Map<String, Object?>>[
        _row(id: '00000000-0000-0000-0000-000000000003', hasMore: false),
        _row(id: '00000000-0000-0000-0000-000000000002', hasMore: false),
        _row(id: '00000000-0000-0000-0000-000000000001', hasMore: false),
      ];

      final page = await _repository(
        _FakeOrdersDataSource(rows),
      ).listOrders(filters: OrderFilters());
    ...
    test('rejects invalid limits before calling the data source', () async {
      final dataSource = _FakeOrdersDataSource(const []);
...
SupabaseCustomerOrdersRepository _repository(
  ...
  return SupabaseCustomerOrdersRepository(
    ...
    readCoordinator: const _ImmediateReadCoordinator(),
  );
}

class _ImmediateReadCoordinator implements PlatformReadCoordinator {
  const _ImmediateReadCoordinator();

  @override
  Future<T> runRead<T>(PlatformReadOperation<T> operation) => operation();
}

class _FakeOrdersDataSource implements SupabaseOrdersDataSource {
  _FakeOrdersDataSource(this.rows);
