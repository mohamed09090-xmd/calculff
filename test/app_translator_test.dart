...
void main() {
  ...
  test('يترجم العناوين الأساسية إلى الفرنسية', () {
    ...
    expect(AppTranslator.translateForLanguage('الإعدادات', 'fr'), 'Paramètres');
  });
  ...
  test('يترجم القيم الديناميكية مع إبقاء الأرقام', () {
    ...
    expect(AppTranslator.translateForLanguage('5 عملية', 'fr'), '5 opérations');
    expect(
  ...
  testWidgets('يعرض النص الفرنسي باتجاه LTR', (tester) async {
    await tester.pumpWidget(
      const material.MaterialApp(
        ...
        home: material.Scaffold(body: localized.Text('الإعدادات')),
      ),
    );
    ...
    expect(material.Directionality.of(element), material.TextDirection.ltr);
  });
}
